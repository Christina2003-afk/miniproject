...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($password) || empty($confirm_password)) {
        ...
        $stmt = $conn->prepare("SELECT reset_token, token_expiry FROM table_reg WHERE reset_token = ?");
        ...
        if ($result->num_rows === 1) {
            ...
            if ($current_time > $token_expiry) {
                ...
                $update_stmt = $conn->prepare("UPDATE table_reg SET password = ?, reset_token = NULL, token_expiry = NULL WHERE reset_token = ?");
...
<head>
    ...
    <style>
        ...
        .reset-card button {
            width: 100%;
            padding: 12px 15px;
            background-color:hsla(36, 92.00%, 43.90%, 0.82);
            color: #fff;
            border: none;
            border-radius: 5px;
            font-size: 16px;
        ...
        .reset-card button:hover {
            background-color:rgb(220, 112, 17);
        }
        .reset-card p {
            margin: 15px 0;
            font-size: 14px;
